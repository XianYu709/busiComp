...
export type TableExpose = {
  ...
  exportAllData: (args?: any) => Promise<void>;
  clearData: () => void;
  getRawData: () => any[];
  ...
  setup(props, { expose, attrs, emit }) {
    ...
    const fetch = async (params?: Object) => {
      ...
      try {
        ...
        if (currentRequestSeq !== requestSeq.value) return;
    ...
    const exportAllData = async (args?: any) => {
      if (typeof props?.exportApi === "function") {
        await props.exportApi(args);
        return;
      }

      const normalizeExportColumns = (cols: any[] = []): any[] => {
        return cols
          .filter(col => !["selection", "index", "expand"].includes(col?.type))
          .filter(col => col?.hiddenInExport !== true)
          .map(col => {
            const children = normalizeExportColumns(col?.children || []);
      ...
      const usePage = props?.pagination && Object.keys(props?.pagination).length > 0;
      const dataKey = props?.dataKey || "rows";
      const mergedParams = {
        ...lastFetchParams.value,
      ...
      if (Array.isArray(props.data)) {
        ...
        exportRows = Array.isArray(res?.[dataKey]) ? res[dataKey] : [];
      }

      const exportColumns = normalizeExportColumns(columns.value as any[]);
      if (!exportColumns.length) {
        console.warn("导出失败：未找到可导出的列配置");
        return;
      }

      selfExportToXlsx(
        exportColumns,
        Array.isArray(exportRows) ? toRaw(exportRows) : [],
        (attrs?.title as string) || "导出数据",
    ...
    return () =>
      withDirectives(
        <div class={props.isFull ? "h-full flex flex-col justify-between" : ""}>
          <ElTable
            ref={elRef}
            class={["w-full", props.isFull ? "flex-1" : ""]}
            {...attrs}
            data={tableData.value}
            max-height={props.maxHeight ? props.maxHeight : props.isFull ? '100%' : undefined}
