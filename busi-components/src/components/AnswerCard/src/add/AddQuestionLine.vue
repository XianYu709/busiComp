  ...
  <div v-for="(it, i) in groups" :key="it.id ?? i">
    <div class="flex items-center justify-between select-none mb-1.5">
      <div class="flex items-center">
        <el-select style="width: 120px" placeholder="题型" v-model="it.questionTypeId" :options="typeOptions" @change="
          () => {
            answerTypeChange(it);
            createChildQustion(it);
          }
        " />
        <div v-if="!isEnglishComposition">
          <span class="mx-2">从</span>
          <el-input type="number" style="width: 70px" v-model.number="it.startNum" @change="createChildQustion(it)"
            :disabled="isEnglishComposition || i !== 0 || !it.questionTypeId" :min="0" />
          题

          <span class="mx-2">到</span>
          <el-input type="number" style="width: 70px" v-model.number="it.endNum" @change="createChildQustion(it)"
            :disabled="isEnglishComposition || !it.questionTypeId" :min="it.startNum" />
          题
        </div>
        <span class="mx-2">每题</span>
        <el-input type="number" style="width: 60px" v-model="it.score" :min="0" :disabled="!it.questionTypeId"
      ...
      <div v-if="!isEnglishComposition" class="flex items-center w-60px justify-between text-20px cursor-pointer">
        <ElIcon class="hover:text-primary" @click="addHandler">
          <Plus />
        </ElIcon>
        <ElIcon class="hover:text-primary" @click="minusHandler(i)">
          <Minus />
        </ElIcon>
      </div>
    </div>
...
const createSingleQuestion = (it: any, prefix: number) => {
...
const createChildQustion = (it: any) => {
  ...
  const oldList = Array.isArray(it.questionList) ? it.questionList : [];
  const oldByPrefix = new Map<number, any>(
    oldList
      .map((q: any): [number, any] => [Number(q?.prefix), q])
      .filter((entry: [number, any]) => Number.isFinite(entry[0])),
  );

  const nextList: any[] = [];
  for (let prefix = it.startNum; prefix <= it.endNum; prefix++) {
    const existed = oldByPrefix.get(prefix);
    if (existed) {
      existed.prefix = prefix;
      if ("questionTypeId" in existed) existed.questionTypeId = it.questionTypeId;
      nextList.push(existed);
      continue;
    }

    nextList.push(createSingleQuestion(it, prefix));
  }

  it.questionList = nextList;
};
  ...
  if (props.category.params.type === "BriefQuestion") {
    const childs =
      (it.questionList || [])
        ...
        .flat()
        .filter(Boolean) || [];
    if (childs.length > 0) setEveryFieldValue(childs, "score", e);
    return;
  }
