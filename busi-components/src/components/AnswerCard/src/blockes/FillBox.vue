<template>
  <div
    ...
    <div
      ...
      :data-position-debug-name="getPositionDebugName('row', groupSetting, index)"
      :ref="el => setInfo(el, groupSetting)">
      <!-- 前缀 -->
      <template
        v-if="defaultSetting.prefix || groupSetting.prefix || defaultSetting.prefixType !== 'hide'">
        <div
          class="w-27px text-13px flex justify-center items-center"
          :class="getPrefixBorderClass()">
      ...
      <div
        class="flex"
        :class="groupSetting.direction === 'horizontal' ? 'items-center' : 'flex-col'"
        :data-position-debug-name="getPositionDebugName('content', groupSetting, index)">
        <div
    ...
    <div
      ...
      class="opacity-0 group-hover:opacity-100 absolute top-0 right-0 flex">
      <el-button
        text
        @click="visible = true"
        type="primary"
...
import { ref, computed, watch, useAttrs, nextTick, toRaw } from "vue";
...
type propsType = {
  ...
  everySetting?: (Omit<propsType, "everySetting" | "hoverSetting" | "border" | "groups"> & {
    contentList?: string[];
  })[]; // 每组自定义字符
};
...
const proxySourceMap = new WeakMap<object, object>();
...
const renderGroups = computed(() => {
  ...
  if (base.everySetting?.length) {
    return base.everySetting.map(item => {
      ...
      proxySourceMap.set(proxy, item);
      return proxy;
    });
  }
  return Array.from({ length: base.groups }, () => base);
});
...
const measureTokens = new WeakMap<object, number>();
...
const getMeasureKey = (item: any) => {
  if (!item || typeof item !== "object") return null;
  const source = proxySourceMap.get(item) || toRaw(item);
  return source && typeof source === "object" ? source : null;
};

const isCurrentMeasure = (pageBox: HTMLElement, el: HTMLElement, key: object, token: number) => {
  return (
    pageBox.isConnected &&
    el.isConnected &&
    pageBox.contains(el) &&
    measureTokens.get(key) === token
  );
};

const getPositionDebugName = (target: "row" | "content", groupSetting: any, index: number) => {
  const page = Number(attrs.pageOf ?? 0) + 1;
  const prefix = getPrefixText(groupSetting, index) || index + 1;
  return `FillBox-${target}:p${page}:${prefix}`;
};

const setInfo = async (el: Element | null, item: any) => {
  const pageBox = attrs?.pageBox as HTMLElement | undefined;
  if (!pageBox || !(el instanceof HTMLElement)) return;

  const key = getMeasureKey(item);
  if (!key) return;

  const token = (measureTokens.get(key) || 0) + 1;
  measureTokens.set(key, token);

  await waitForLayoutMeasurement();
  if (!isCurrentMeasure(pageBox, el, key, token)) return;

  const rect: any = await getPosition(pageBox, el, "none");
