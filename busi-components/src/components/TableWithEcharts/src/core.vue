...
// 处理 排名分段分布 下拉选择框数据
const getChartRows = (payload: any): any[] => {
  if (Array.isArray(payload?.rows)) return payload.rows;
  if (Array.isArray(payload?.data)) return payload.data;
  return [];
}

const processRankingData = (payload: any) => {
  const rows = getChartRows(payload)
  if (!rows.length || !rows[0]?.segmentResults) {
    return [];
  }
  const segmentArr = rows[0].segmentResults
...
const parsePercentNumber = (value: any) => {
...
const appendPercentIfMissing = (value: any) => {
...
const initBarChart = async (sledValue: string) => {
  ...
  if (props.barTypeDefine === 'selectBar') {
    // ========== selectBar配置：按排名展示人数 ==========
    const rankingRows = getChartRows(rawData);
    const defaultSegmentName = rankingRows?.[0]?.segmentResults?.[0]?.segmentName;
    const targetSegmentName = sledValue || defaultSegmentName || "";
    const legendName = [targetSegmentName ? `前${targetSegmentName}名` : "人数"];

    const sourceData = [
      ['product', ...legendName],
      ...rankingRows.map((row: { groupName?: string; className?: string; schoolName?: string; segmentResults?: any[]; }) => {
        const segmentList = Array.isArray(row?.segmentResults) ? row.segmentResults : [];
        const targetSegment = targetSegmentName
          ? segmentList.find((item) => item?.segmentName === targetSegmentName)
          : segmentList[0];
        return [
          row?.groupName || row?.className || row?.schoolName || "",
          Number(targetSegment?.count) || 0
        ];
      })
    ];
    ...
    // ========== bar1配置：按班级和等级展示百分比 ==========
    const chartData = getChartRows(rawData);
    ...
    const seriesData = chartData.map((cls: any) => {
      const levelMap = cls.levelDistributions?.reduce((map: Record<string, { ratio: number; count: number }>, level: any) => {
        map[level.levelName] = {
          ratio: parsePercentNumber(level.segmentRatio),
      ...
      tooltip: {
        ...
        formatter: function (params: any[]) {
          ...
          params.forEach((param: any) => {
            ...
            html += `<div>${levelName}: ${appendPercentIfMissing(ratio)} (${count}人)</div>`;
          });
          return html;
        },
    ...
    if (rawData?.rows?.length) {
      chartSource = [
        ['product', '最高分', '最低分', '平均分'], // 表头
        ...rawData.rows.map((row: { schoolName: any; subjectName: any; groupName: any; maxScore: any; minScore: any; avgScore: any; }) => [
          row.schoolName ? row.schoolName + '-' + row?.subjectName : row.groupName ? row.groupName + (row.subjectName ? '-' + row.subjectName : '') : row.subjectName,
          row.maxScore, // 最高分
          row.minScore, // 最低分
          row.avgScore // 平均分
        ])
...
const initlineChart = async () => {
  ...
  const seriesList = chartData.map((groupItem: any, index: number) => {
    ...
    const ratioData =
      groupItem.segmentResults?.map((item: any) => appendPercentIfMissing(item.ratio || "0.00")) || [];
      ...
      formatter: function (params: any[]) {
        ...
        params.forEach((param: any) => {
          ...
          tooltipHtml += `<div>
            ...
            ${groupName}：${count}人，比例${appendPercentIfMissing(ratio)}
          </div>`;
        });
        return tooltipHtml;
      }
...
const initRadarChart = async () => {
  ...
  const nameData: any[] = []
  inteData.rows.forEach((item: { subjectName: string; subjectScore: number; totalScore: number; className: string }) => {
    if(dataExample.findIndex((examitem) => examitem.name === item?.subjectName) === -1) {
      dataExample.push({
        name: item?.subjectName || "",
        max: item?.totalScore || item.subjectScore + 10
      });
    }
    if(valueData.findIndex((valueItem) => valueItem.name === item?.className) === -1) {
      valueData.push({
        value: [],
        name: item?.className || ""
      });
    ...
    valueData.forEach((valueItem) => {
      if(valueItem.name == item?.className) {
        valueItem.value.push(item?.subjectScore);
      }
    })
    // valueData.push({
