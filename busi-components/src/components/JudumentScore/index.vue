...
const cloneRules = <T,>(value: T): T => JSON.parse(JSON.stringify(value));
...
const multipleForm = ref<any>(cloneRules(DEFAULT_RULES));

const hasRules = computed(() => {
  const rules = props.rules as Record<string, any> | undefined;
  if (!rules || typeof rules !== "object") return false;
  if (!("value" in rules)) return false;
  if (String(rules.value) === "2") return true;
  return "score" in rules;
});
...
watch(
  ...
  val => {
    if (hasRules.value) {
      multipleForm.value = cloneRules(val);
      return;
    }
    multipleForm.value = cloneRules(DEFAULT_RULES);
    if (props.immediate) {
      emit("update:rules", cloneRules(DEFAULT_RULES));
    }
  },
  {
    deep: true,
...
const closeJudument = (isSave: boolean = false) => {
  if (isSave) {
    emit("update:rules", cloneRules(multipleForm.value));
  }
