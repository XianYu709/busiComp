...
export const getStudentList = (params: {
  examInfoId: string | number;
  pageSize?: number;
  pageNum?: number;
  studentCode?: string;
}) => {
  return request({
    url: '/generality-center/academi/student/list',
    method: 'get',
    params
  })
}
 ...
 * 删除作业异常记录
 */
export const deleteHomeworkScanError = (ids: string) => {
  return request({
    url: `/learning-center/homework/scanRecordError/${ids}`,
    method: 'delete',
  })
}
 ...
 * 删除考试异常记录
 */
export const deleteExamScanError = (ids: string) => {
  return request({
    url: `/learning-center/exam/scanRecordError/${ids}`,
    method: 'delete',
  })
}
 ...
 * 获取学号异常答题卡图片
 */
export const getOssImage = (ossid: string) => {
  return request({
    url: '/system-center/resource/oss/outer/selectSysOssVoOnlyBusinessId/' + ossid,
    method: "GET"
  })
}
