      ...
      <div class="flex-1 flex overflow-hidden">
        ...
        <div class="flex-1 flex flex-col border shadow-sm relative z-0 m-5 overflow-hidden">
          ...
          <div class="h-10 border-b flex items-center justify-between px-4 bg-gray-50 select-none">
            <div class="flex items-center gap-4 text-gray-600 mr-4">
              <span class="text-xs text-gray-500 mt-1">姓名：{{ studentInfo.studentName }}</span>
        ...
        <div
          ...
          <div class="flex-1 overflow-y-auto custom-scrollbar" style="border: 1px solid #ccc">
            <div v-if="activeTab === 'objective'" class="m-2 border-gray-300 rounded overflow-hidden">
              <el-table
                :data="objectiveQuestions"
                :header-cell-style="{ background: '#F9FAFE', color: '#333', borderRight: '1px solid #ddd', borderBottom: '1px solid #ddd' }"
                :cell-style="{ borderRight: '1px solid #ddd', borderBottom: '1px solid #ddd' }"
                :row-class-name="tableRowClassName"
                style="width: 100%"
                :show-header="true"
                :default-sort="{ prop: 'questionNo', order: 'ascending' }">
                <el-table-column prop="questionNo" label="题号" width="80" align="center" header-align="center" :resizable="false">
                  <template #default="{ row }">
                    <span class="text-sm font-mono text-gray-800">{{ row.questionNo }}</span>
                  </template>
                </el-table-column>
                <el-table-column prop="selectedOption" label="选项" header-align="left" :resizable="false">
                  <template #default="{ row }">
              ...
              <div v-if="objectiveQuestions.length === 0" class="h-40 flex flex-col items-center justify-center text-gray-400 border-t border-gray-300">
                <el-empty description="暂无客观题数据" :image-size="60" />
              </div>
            </div>
            <div v-else class="m-2 border border-gray-300 rounded overflow-hidden" style="border: 1px solid #ccc">
              <div class="flex flex-col">
                <div v-for="item in subjectiveQuestions" :key="item.questionNo" class="border-b border-gray-200 p-3">
                  <div class="flex items-center justify-between mb-3">
                    <span class="font-medium text-gray-700">{{ item.questionNo }}</span>
                    <div class="flex items-center gap-3">
                      <span class="text-xs text-blue-600 cursor-pointer hover:text-blue-800" @click="handleUploadImage(item)">上传图片</span>
                      <div class="h-3 w-px bg-gray-300"></div>
                      <span class="text-xs text-blue-600 cursor-pointer hover:text-blue-800" @click="handleDeleteImage(item)">删除</span>
                    </div>
                  </div>
                  <div
                    ...
                    <img v-if="item.imageUrl" :src="item.imageUrl" class="w-full h-auto max-h-48 object-contain p-1" alt="主观题图片" @error="handleImageError(item)" />
                    <div v-else class="w-full h-10 flex items-center justify-center"></div>
                  </div>
                </div>
                <div v-if="subjectiveQuestions.length === 0" class="h-40 flex flex-col items-center justify-center text-gray-400">
...
import { ArrowLeft, UploadFilled, Loading, Delete, Edit } from "@element-plus/icons-vue";
...
const paperList = reactive([
  {
    studentName: (route.query.studentName as string) || "",
    studentCode: (route.query.studentCode as string) || "",
...
const studentInfo = computed(() => ({
  studentName: paperList[currentPaperIndex.value]?.studentName,
  studentCode: paperList[currentPaperIndex.value]?.studentCode,
}));
...
const prevPaper = () => {
  if (!isFirstPaper.value) {
    ...
    ElMessage.success(`已切换到【${studentInfo.value.studentName}】的答题卡`);
  }
};
