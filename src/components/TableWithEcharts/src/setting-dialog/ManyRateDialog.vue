...
// 获取默认配置
const getDefaultConfig = (mode: 'independent' | 'cumulative', count: 4 | 5): Grade[] => {
  // 基础档次配置（根据数量过滤）
  const baseGrades = [
  ...
  if (count === 5) {
    ...
    // 再添加新的最高等级
    baseGrades.unshift({ levelName: '高分', isMax: true, isLowest: false });
  } else {
    baseGrades.slice(0, count);
  }

  // 根据模式设置不同的默认值
  if (mode === 'independent') {
    return baseGrades.map((grade, index, array) => {
...
// 处理档线数量变更
const handleGradeCountChange = (count: 4 | 5) => {
