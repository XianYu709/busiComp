export function getPosition(
  ...
  return new Promise(resolve => {
    requestAnimationFrame(() => {
      ...
      if (testType !== "none") {
        // 清除旧测试元素
        absolute.querySelectorAll(".__position_test_mark__").forEach(el => el.remove());
...
export function getPositionWithCache(
  ...
  return new Promise(resolve => {
    requestAnimationFrame(() => {
      ...
      if (entry.pending && entry.testType === testType) {
      ...
      entry.pending = new Promise<PositionReturn>(innerResolve => {
        ...
        // ✅ 只有重新测量才画测试标记
        applyTestMark(relative, absolute, testType, result.pixel, rectA);
