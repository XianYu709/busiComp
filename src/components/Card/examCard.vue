...
<template>
  <div class="card-top">
    ...
    <div class="card-top-title">
      ...
      <span class="card-top-title-t">考试空答题卡</span>
    ...
    <div class="card-content-sheet" id="card-content-sheet">
      <el-scrollbar>
        ...
        <div class="page" id="page">
          <div class="exam-title" id=""><h3>XXX考试空答题卡</h3></div>
          <div id="cardContent" class="card-container" :style="{height: paperHeight+'px'}">
            <div class="top-info-d" v-if="precautions">
              <div class="top-info" style="">
                <span class="" style="">姓名：</span>
                <p class="" style=""></p>
              </div>
            ...
            <div class="card-content-sheet-top">
              <div class="card-content-sheet-top-basic-information" id="cardInfo">
                ...
                <div class="information" v-if="!precautions">
                ...
                <div v-if="precautions">
                  <p v-for="(item,index) in PRECAUTIONS" :key="index" class="precautions">{{ index + 1 }}.{{item}}</p>
                </div>
              </div>
              ...
              <!--二维码图片-->
              <div style="width: 195px;height: 195px" class="card-content-sheet-top-qrcode" v-if="isShowQRCode">
            ...
            <div v-if="isAbsent" class="absent" >
              ...
              <div class="absent-r" >缺考标记：【 &nbsp;&nbsp;】&nbsp;&nbsp; 缺考考生由监考员填涂考号，并用2B铅笔填涂缺考标记</div>
            </div>
            <!-- 答题卡内容 -->
...
<!--              <template #title>
            ...
            <div style="margin: 10px 24px">
              ...
              <div class="card-content-config-layout-paper">
                <span>{{ layoutPaperSize }}</span>
                <el-button type="primary" text @click="modifyPageLayout">修改</el-button>
              </div>
            </div>
<!--            </el-collapse-item>-->
<!--            <p class="card-content-config-title">答题卡基础信息</p>-->
            <el-collapse-item name="2" style="width: 97%">
              <template #title>
                <span class="card-content-config-layout">答题卡基础信息</span>
              </template>
              <div>
                <span style="margin-left: 24px">考号板式：</span>
                <el-radio-group v-model="radio" style="width: 230px">
                  <el-radio :value="3" @change="onChangeExamNumber('fill')">考号填涂</el-radio>
                  <el-radio :value="6" @change="onChangeExamNumber('paste')">条形码</el-radio>
                </el-radio-group>
              </div>
              <div class="card-content-config-layout-box" style="margin-left: 24px">
                <el-checkbox v-model="precautions" label="注意事项" size="large" />
                ...
                <el-checkbox v-model="arrangement" label="考号竖向排列" size="large" @change="changeExamNumberArrangement"/>
                <el-checkbox v-model="isShowQRCode" label="二维码" size="large" @change="changeShowQRCode"/>
              </div>

            </el-collapse-item>
            <el-collapse-item name="3" style="width: 97%">
              <template #title>
                <span class="card-content-config-layout">题目显示方式</span>
              </template>
              <div class="" style="margin-left: 24px ">
                ...
                <el-checkbox v-model="isMergeSubjective" size="large" @change="changeMergeSubjective">主观题小题合并批阅
                  <el-tooltip
                      class="box-item"
                      effect="dark"
                ...
                <el-checkbox v-model="checked3" label="填空题合并批阅后按题打分" size="large" />
...
//主观题合并
const isMergeSubjective = ref(false)
const changeMergeSubjective = (val:any)=>{

}
//二维码
const isShowQRCode = ref(false)
const changeShowQRCode = (val)=>{
...
//改变考号排列
const changeExamNumberArrangement = (val:boolean)=>{
  console.log(val);
  const ss = document.getElementById('examNumber');
  //在选择了考号填涂的前提下
  if(boolFill.value){
    if(val){
      //添加注意事项
      ss.style.display = 'flex';
      for (let child of document.getElementById('examNumber').children) {
        child.style.width = '30px';
        child.style.lineHeight = '19px';
      }
...
const downloadBtn = () => {
  ...
  link.download = '我的答题卡.png'; // 设置下载文件名
